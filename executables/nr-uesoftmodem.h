...
#define  CONFIG_HLP_ACTOR_CORES            "Comma-separated cores for DL then UL actors (and SYNC if one spare). Must have at least num-dl-actors + num-ul-actors entries; omit for no affinity\n"
...
#define CMDLINE_NRUEPARAMS_DESC {  \
  ...
  {"actor-cores",                  CONFIG_HLP_ACTOR_CORES,     0,               .strptr=&nrUE_params.actor_cores,           .defstrval=NULL,   TYPE_STRING,   0}, \
...
typedef struct {
  ...
  int num_ul_actors;
  int num_dl_actors;
  /* Shared core pool for actors (see --actor-cores). NULL means leave them unpinned. */
  char *actor_cores;
  int extra_pdu_id;
} nrUE_params_t;
